...
public class StickyHeaderLayout extends FrameLayout {
    ...
    private void addOnScrollListener() {
        mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                final GroupedRecyclerViewAdapter gAdapter = (GroupedRecyclerViewAdapter) adapter;
                ...
                if (isSticky && gAdapter.isCollapseGroupp(groupPosition)) {
                    mStickyLayout.setVisibility(VISIBLE);
                    if (isScrollUp) {
                        updateStickyView(true, "onScrolled");
                    } else {
                        //if(gAdapter.judgeType(firstVisibleItem) == TYPE_HEADER){
                        updateStickyView(true, "onScrolled");
                        //}
                    }
     ...
     * 强制更新吸顶布局。
     */
    public void updateStickyView(String str) {
        updateStickyView(true, str);
    }
     ...
     */
    private void updateStickyView(boolean imperative, String str) {
        Log.d("ouyang", "str : " + str);
        RecyclerView.Adapter adapter = mRecyclerView.getAdapter();
        //只有RecyclerView的adapter是GroupedRecyclerViewAdapter的时候，才会添加吸顶布局。
        if (adapter instanceof GroupedRecyclerViewAdapter) {
            ...
            if ((imperative || mCurrentStickyGroup != groupPosition)) {
                ...
                if (groupHeaderPosition != -1 && gAdapter.isCollapseGroupp(groupPosition)) {
                    //获取吸顶布局的viewType。
                    int viewType = gAdapter.getItemViewType(groupHeaderPosition);
                    if (viewType == GroupedRecyclerViewAdapter.FOOTER_VIEW)
                        return;
        ...
        if (!isRegisterDataObserver) {
            ...
            adapter.registerAdapterDataObserver(new RecyclerView.AdapterDataObserver() {
                ...
                public void onChanged() {
                    updateStickyViewDelayed("onChanged");
                }

                @Override
                public void onItemRangeChanged(int positionStart, int itemCount) {
                    updateStickyViewDelayed("onItemRangeChanged");
                }

                @Override
                public void onItemRangeInserted(int positionStart, int itemCount) {
                    updateStickyViewDelayed("onItemRangeInserted");
                }

                @Override
                public void onItemRangeRemoved(int positionStart, int itemCount) {
                    updateStickyViewDelayed("onItemRangeRemoved");
                }
    ...
    private void updateStickyViewDelayed(final String str) {
    ...
    public void setSticky(boolean sticky) {
        if (isSticky != sticky) {
            ...
            if (mStickyLayout != null) {
                if (isSticky) {
                    mStickyLayout.setVisibility(VISIBLE);
                    updateStickyView(false, "setSticky");
                } else {
                    recycle();
                    mStickyLayout.setVisibility(GONE);
                }
