...
import com.happy.todo.lib_http.api.ApiService;
...
import com.happy.todo.lib_http.interceptor.CacheInterceptor;
import com.happy.todo.lib_http.interceptor.CacheInterceptorOffline;
import com.happy.todo.lib_http.interceptor.HeadersInterceptor;
import com.happy.todo.lib_http.interceptor.NoCacheInterceptor;
...
import static com.happy.todo.lib_http.IFreeHttp.getRxCache;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    public BaseRequest(String url) {
        this.url = url;
        context = IFreeHttp.getContext();
        IFreeHttp config = IFreeHttp.getInstance();
    ...
    public R cacheTime(long cacheTime) {
        if (cacheTime <= -1) cacheTime = IFreeHttp.DEFAULT_CACHE_NEVER_EXPIRE;
    ...
    private OkHttpClient.Builder generateOkClient() {
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && sslParams == null
            ...
            OkHttpClient.Builder builder = IFreeHttp.getOkHttpClientBuilder();
            ...
            final OkHttpClient.Builder newClientBuilder = IFreeHttp.getOkHttpClient().newBuilder();
            if (readTimeOut > 0)
            ...
            if (cookies.size() > 0) IFreeHttp.getCookieJar().addCookies(cookies);
    ...
    private Retrofit.Builder generateRetrofit() {
        if (converterFactories.isEmpty() && adapterFactories.isEmpty()) {
            return IFreeHttp.getRetrofitBuilder().baseUrl(baseUrl);
        } else {
            ...
            if (!converterFactories.isEmpty()) {
                ...
                //获取全局的对象重新设置IFreeHttp
                List<Converter.Factory> listConverterFactory = IFreeHttp.getRetrofit().converterFactories();
            ...
            if (!adapterFactories.isEmpty()) {
                ...
                //获取全局的对象重新设置
                List<CallAdapter.Factory> listAdapterFactory = IFreeHttp.getRetrofit().callAdapterFactories();
    ...
    private RxCache.Builder generateRxCache() {
        final RxCache.Builder rxCacheBuilder = IFreeHttp.getRxCacheBuilder();
        switch (cacheMode) {
            case NO_CACHE://不使用缓存
            ...
            case DEFAULT://使用Okhttp的缓存
                if (this.cache == null) {
                    File cacheDirectory = IFreeHttp.getCacheDirectory();
                    if (cacheDirectory == null) {
                        cacheDirectory = new File(IFreeHttp.getContext().getCacheDir(), "okhttp-cache");
                    } else {
                        if (cacheDirectory.isDirectory() && !cacheDirectory.exists()) {
                            cacheDirectory.mkdirs();
                        }
                    }
                    this.cache = new Cache(cacheDirectory, Math.max(5 * 1024 * 1024, IFreeHttp.getCacheMaxSize()));
                }
                String cacheControlValue = String.format("max-age=%d", Math.max(-1, cacheTime));
                final CacheInterceptor REWRITE_CACHE_CONTROL_INTERCEPTOR = new CacheInterceptor(IFreeHttp.getContext(), cacheControlValue);
                final CacheInterceptorOffline REWRITE_CACHE_CONTROL_INTERCEPTOR_OFFLINE = new CacheInterceptorOffline(IFreeHttp.getContext(), cacheControlValue);
