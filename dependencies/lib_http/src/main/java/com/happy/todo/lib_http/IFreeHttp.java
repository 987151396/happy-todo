package com.happy.todo.lib_http;
...
import com.happy.todo.lib_http.cache.converter.IDiskConverter;
import com.happy.todo.lib_http.cache.converter.SerializableDiskConverter;
import com.happy.todo.lib_http.cache.model.CacheMode;
import com.happy.todo.lib_http.cookie.CookieManger;
import com.happy.todo.lib_http.https.HttpsUtils;
import com.happy.todo.lib_http.interceptor.HttpLoggingInterceptor;
import com.happy.todo.lib_http.model.HttpHeaders;
import com.happy.todo.lib_http.model.HttpParams;
import com.happy.todo.lib_http.request.CustomRequest;
import com.happy.todo.lib_http.request.DeleteRequest;
...
import com.happy.todo.lib_http.request.PutRequest;
 ...
 * 11.支持Cookie管理</br>
 * <p>
 * EEADME: https://github.com/zhou-you/RxEasyHttp
 */

public class IFreeHttp {
    ...
    private String mRecommendedHotelUrl;                              //推荐酒店BaseUrl
    ...
    private volatile static IFreeHttp singleton = null;
    ...
    public static IFreeHttp getInstance() {
        testInitialize();
        if (singleton == null) {
            synchronized (IFreeHttp.class) {
                if (singleton == null) {
                    singleton = new IFreeHttp();
                }
            }
        }
        return singleton;
    ...
    private static void testInitialize() {
        if (sContext == null)
            throw new ExceptionInInitializerError("请先在全局Application中调用 IFreeHttp.init() 初始化！");
    }
     ...
     * 调试模式,默认打开所有的异常调试
     */
    public IFreeHttp debug(String tag) {
        debug(tag, true);
        return this;
    }
     ...
     */
    public IFreeHttp debug(String tag, boolean isPrintException) {
        HttpLog.customTagPrefix = TextUtils.isEmpty(tag) ? "IFREE_HTTP" : tag;
     ...
     * https的全局访问规则
     */
    public IFreeHttp setHostnameVerifier(HostnameVerifier hostnameVerifier) {
     ...
     * https的全局自签名证书
     */
    public IFreeHttp setCertificates(InputStream... certificates) {
     ...
     * https的全局自签名证书
     */
    public IFreeHttp setSSLSocketFactory(SSLSocketFactory sslSocketFactory,
     ...
     * https双向认证证书
     */
    public IFreeHttp setCertificates(InputStream bksFile, String password, InputStream...
            certificates) {
     ...
     * 全局cookie存取规则
     */
    public IFreeHttp setCookieStore(CookieManger cookieManager) {
     ...
     * 全局读取超时时间
     */
    public IFreeHttp setReadTimeOut(long readTimeOut) {
     ...
     * 全局写入超时时间
     */
    public IFreeHttp setWriteTimeOut(long writeTimeout) {
     ...
     * 全局连接超时时间
     */
    public IFreeHttp setConnectTimeout(long connectTimeout) {
     ...
     * 超时重试次数
     */
    public IFreeHttp setRetryCount(int retryCount) {
     ...
     * 超时重试延迟时间
     */
    public IFreeHttp setRetryDelay(int retryDelay) {
     ...
     * 超时重试延迟叠加时间
     */
    public IFreeHttp setRetryIncreaseDelay(int retryIncreaseDelay) {
     ...
     * 全局的缓存模式
     */
    public IFreeHttp setCacheMode(CacheMode cacheMode) {
     ...
     * 全局的缓存过期时间
     */
    public IFreeHttp setCacheTime(long cacheTime) {
     ...
     * 全局的缓存大小,默认50M
     */
    public IFreeHttp setCacheMaxSize(long maxSize) {
     ...
     */
    public IFreeHttp setCacheVersion(int cacheersion) {
        if (cacheersion < 0)
     ...
     */
    public IFreeHttp setCacheDirectory(File directory) {
     ...
     * 全局设置缓存的转换器
     */
    public IFreeHttp setCacheDiskConverter(IDiskConverter converter) {
     ...
     * 全局设置OkHttp的缓存,默认是3天
     */
    public IFreeHttp setHttpCache(Cache cache) {
        this.mCache = cache;
        return this;
    }
     ...
     * 添加全局公共请求参数
     */
    public IFreeHttp addCommonParams(HttpParams commonParams) {
     ...
     * 添加全局公共请求参数
     */
    public IFreeHttp addCommonHeaders(HttpHeaders commonHeaders) {
     ...
     * 添加全局拦截器
     */
    public IFreeHttp addInterceptor(Interceptor interceptor) {
     ...
     * 添加全局网络拦截器
     */
    public IFreeHttp addNetworkInterceptor(Interceptor interceptor) {
        okHttpClientBuilder.addNetworkInterceptor(Utils.checkNotNull(interceptor, "interceptor ==" +
                " null"));
        return this;
    }
     ...
     * 全局设置代理
     */
    public IFreeHttp setOkproxy(Proxy proxy) {
     ...
     * 全局设置请求的连接池
     */
    public IFreeHttp setOkconnectionPool(ConnectionPool connectionPool) {
        okHttpClientBuilder.connectionPool(Utils.checkNotNull(connectionPool, "connectionPool == " +
                "null"));
        return this;
    }
     ...
     */
    public IFreeHttp setOkclient(OkHttpClient client) {
     ...
     */
    public IFreeHttp addConverterFactory(Converter.Factory factory) {
     ...
     */
    public IFreeHttp addCallAdapterFactory(CallAdapter.Factory factory) {
     ...
     */
    public IFreeHttp setCallbackExecutor(Executor executor) {
     ...
     */
    public IFreeHttp setCallFactory(okhttp3.Call.Factory factory) {
     ...
     * 全局设置baseurl
     */
    public IFreeHttp setBaseUrl(String baseUrl) {
     ...
     * 全局设置推荐酒店baseurl
     */
    public IFreeHttp setRecommendedHotelUrl(String mRecommendedHotelUrl) {
        this.mRecommendedHotelUrl =  Utils.checkNotNull(mRecommendedHotelUrl, "mRecommendedHotelUrl == null");
        return this;
    }
    /**
     * 获取全局推荐酒店baseurl
     */
    public String getRecommendedHotelUrl() {
        return mRecommendedHotelUrl;
    }
     ...
     * 批量取消订阅
     */
    public static void cancelDisposables(Disposable... disposables) {
        if (disposables != null) {
