 ...
 * {@link CalendarPickerView.FluentInitializer} methods returned.  The currently selected date can be retrieved with
 * {@link #getSelectedDate()}.
 * Created by Jaminchanks on 2018/8/6.
 */
public class CalendarPickerView extends RecyclerView {
    ...
    private boolean mIsShowUsefulWeeks = true; //是否从所在时间的当个星期开始显示
...
//        setCacheColorHint(bg);
        ...
        setDrawingCacheBackgroundColor(bg);
     ...
     * <p>
     * This will implicitly set the {@link CalendarSelectionMode} to {@link CalendarSelectionMode#SINGLE}.  If you
     * want a different selection mode, use {@link FluentInitializer#inMode(CalendarSelectionMode)} on the
    ...
    public FluentInitializer init(Date minDate, Date maxDate, TimeZone timeZone, Locale locale) {
        ...
        while ((monthCounter.get(MONTH) <= maxMonth // Up to, including the calendar_month.
            ...
            Logr.d("Adding calendar_month %s", month);
            months.add(month);
    ...
    public class FluentInitializer {
        /**
         * Override the {@link CalendarSelectionMode} from the default ({@link CalendarSelectionMode#SINGLE}).
         */
        public FluentInitializer inMode(CalendarSelectionMode mode) {
         ...
         * pass in multiple dates and haven't already called {@link #inMode(CalendarSelectionMode)}.
         */
        public FluentInitializer withSelectedDates(Collection<Date> selectedDates) {
            if (calendarSelectionMode == CalendarSelectionMode.SINGLE && selectedDates.size() > 1) {
                throw new IllegalArgumentException("SINGLE mode can't be used with multiple selectedDates");
            }
            if (calendarSelectionMode == CalendarSelectionMode.RANGE && selectedDates.size() > 2) {
                throw new IllegalArgumentException(
     ...
     *
     * @param date 所选时间
     * @param isSmooth 是否圆滑滚动
     * @return
     */
    public boolean scrollToDate(Date date, boolean isSmooth) {
        Integer selectedIndex = null;
        ...
        if (selectedIndex != null) {
            scrollToSelectedMonth(selectedIndex, isSmooth);
            return true;
        }
        return false;
    }
...
//        }
    ...
    /**
     * Select a new date.  Respects the {@link CalendarSelectionMode} this CalendarPickerView is configured
     * with: if you are in {@link CalendarSelectionMode#SINGLE}, the previously selected date will be
     * un-selected.  In {@link CalendarSelectionMode#MULTIPLE}, the new date will be added to the list of
     * selected dates.
     * <p>
    ...
    private boolean doSelectDate(Date date, MonthCellDescriptor cell) {
        ...
        switch (calendarSelectionMode) {
            ...
            case SINGLE:
                ...
                break;
            default:
                throw new IllegalStateException("Unknown calendarSelectionMode " + calendarSelectionMode);
        }
        ...
        if (date != null) {
            ...
            if (calendarSelectionMode == CalendarSelectionMode.RANGE && selectedCells.size() > 1) {
    ...
    private class MonthAdapter extends Adapter<MonthViewHolder> {
        ...
        public void onBindViewHolder(@NonNull MonthViewHolder holder, int position) {
            holder.itemView.setDecorators(decorators);
            if (monthsReverseOrder) {
                position = months.size() - position - 1;
            }
            holder.itemView.init(months.get(position), cells.getValueAtIndex(position), displayOnly,
    ...
    private class MonthViewHolder extends ViewHolder {
        ...
        private MonthViewHolder(MonthView itemView) {
            super(itemView);
    ...
    /**
     * 设置是否只显示需要可选择的部分
     * @param isOnlyShow true: 只显示需要可选择的部分 false:显示完整月份的所有日期
     * @param isValidate
     */
    public void setIsOnlyShowUsefulWeeks(boolean isOnlyShow, boolean isValidate) {
        this.mIsShowUsefulWeeks = isOnlyShow;
    ...
    List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month, Calendar startCal) {
        ...
        //如果需要日历的最初显示时间从最小可选的所在星期开始，则不要将该月的最早可显示日期设置为1
        //超过当前年份或者当前月份小于等于日历最小显示月份的当前月份不显示当天之前的时间
        if (cal.get(YEAR) > minCal.get(YEAR) || !(mIsShowUsefulWeeks && cal.get(MONTH) <= minCal.get(MONTH))) {
        ...
        while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month.getYear()) //日期符合日历逻辑
                && cal.get(YEAR) <= month.getYear() //同上
                && (!mIsShowUsefulWeeks || cal.getTime().before(maxCal.getTime())) //只显示可用的星期
                ) {
            Logr.d("Building calendar_week row starting at %s", cal.getTime());
