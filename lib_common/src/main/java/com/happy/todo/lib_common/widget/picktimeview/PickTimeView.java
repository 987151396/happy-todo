package com.happy.todo.lib_common.widget.picktimeview;
...
public class PickTimeView extends LinearLayout implements NumberPicker.OnValueChangeListener, NumberPicker.OnScrollListener {
...
//        mNpRight = new NumberPicker(mContext);
        ...
        setNumberPickerDivider(mNpLeft);
        setNumberPickerDivider(mNpMiddle);
        setNumberPickerDivider(mNpRight);
...
//        mNpRight.setDividerColor(Color.GREEN);
        ...
        mNpLeft.setOnScrollListener(this);
        mNpMiddle.setOnScrollListener(this);
        mNpRight.setOnScrollListener(this);
    ...
    private void updateLeftValue(long timeMillis) {
        ...
        if (mCurrentType == TYPE_PICK_DATE) {
            ...
            sdf = new SimpleDateFormat(ResourceUtils.getString(R.string.date_type_plane2));
            for (int i = 0; i < DATA_SIZE; i++) {
                Calendar cal = Calendar.getInstance();
                cal.setTimeInMillis(timeMillis);
    ...
    public void onValueChange(NumberPicker picker, int oldVal, int newVal) {
         ...
         * 向外部发送当前选中时间
         */
        Calendar calendar_after48  = DateUtil.getCalendarForLocale(Calendar.getInstance(), Locale.getDefault());
        calendar_after48.set(Calendar.HOUR_OF_DAY, calendar_after48.get(Calendar.HOUR_OF_DAY) + 48);

        if (calendar.getTimeInMillis() < calendar_after48.getTimeInMillis()){
           /* updateLeftValue(now_Calendar1.getTimeInMillis());
            updateMiddleValue(now_Calendar1.getTimeInMillis());
            updateRightValue(now_Calendar1.getTimeInMillis());*/
            if (mOnSelectedChangeListener != null) {
                mOnSelectedChangeListener.onSelected(this,calendar_after48.getTimeInMillis());
            }
            mHandler.removeMessages(666);
            mHandler.sendEmptyMessageDelayed(666,500);
        }else{
            if (mOnSelectedChangeListener != null) {
    ...
    private Handler mHandler = new Handler(){
        ...
        public void dispatchMessage(Message msg) {
            if(msg.what == 666){
                PickTimeView.this.removeAllViews();
                generateView();
                initPicker();
            }
        }
    ...
    public void onScrollStateChange(NumberPicker view, int scrollState) {
        switch (scrollState){
            case NumberPicker.OnScrollListener.SCROLL_STATE_FLING:
                AppLogUtil.d("ouyang","scrollState : " + SCROLL_STATE_FLING);
                break;
            case NumberPicker.OnScrollListener.SCROLL_STATE_IDLE:
               /* Calendar now_Calendar1 = DateUtil.getCalendarForLocale(Calendar.getInstance(), Locale.getDefault());
                if (getCalendar.getTimeInMillis() < now_Calendar1.getTimeInMillis()){
                    updateLeftValue(now_Calendar1.getTimeInMillis());
                    updateMiddleValue(now_Calendar1.getTimeInMillis());
                    updateRightValue(now_Calendar1.getTimeInMillis());
                }*/
                AppLogUtil.d("ouyang","scrollState : " + SCROLL_STATE_IDLE);
                break;
            case NumberPicker.OnScrollListener.SCROLL_STATE_TOUCH_SCROLL:
                AppLogUtil.d("ouyang","scrollState : " + SCROLL_STATE_TOUCH_SCROLL);
                break;
        }
    }
