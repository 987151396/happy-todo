...
/**
 * @作者: TwoSX
 * @时间: 2017/12/3 下午12:00
 * @描述: SpannableString 相关工具类
 */
public class SpanUtil {
     ...
     * @return {@link SpanUtil}
     */
    public SpanUtil setLineHeight(@IntRange(from = 0) final int lineHeight, @Align final int
            align) {
     ...
     * @return {@link SpanUtil}
     */
    public SpanUtil setQuoteColor(@ColorInt final int color, @IntRange(from = 1) final int
            stripeWidth, @IntRange(from = 0) final int gapWidth) {
     ...
     * @return {@link SpanUtil}
     */
    public SpanUtil setLeadingMargin(@IntRange(from = 0) final int first, @IntRange(from = 0)
    final int rest) {
        this.first = first;
        this.rest = rest;
        return this;
     ...
     * @return {@link SpanUtil}
     */
    public SpanUtil setBullet(@ColorInt final int color, @IntRange(from = 0) final int radius,
     ...
     * @return {@link SpanUtil}
     */
    public SpanUtil setIconMargin(final Drawable drawable, final int gapWidth, @Align final int
            align) {
     ...
     * @return {@link SpanUtil}
     */
    public SpanUtil setIconMargin(@DrawableRes final int resourceId, final int gapWidth, @Align
    final int align) {
    ...
    public SpanUtil setIconMargin(@DrawableRes final int resourceId, final int gapWidth, @Align
    final int align, int iconMaxWidth, int iconMaxHeight) {
        this.iconMarginResourceId = resourceId;
        this.iconMarginGapWidth = gapWidth;
        this.alignIconMargin = align;
     ...
     * @return {@link SpanUtil}
     */
    public SpanUtil setFontProportion(@FloatRange(from = 0, fromInclusive = false) final float
                                              proportion) {
     ...
     * @return {@link SpanUtil}
     */
    public SpanUtil setFontXProportion(@FloatRange(from = 0, fromInclusive = false) final float
                                               proportion) {
    ...
    private void updateCharCharSequence() {
        ...
        if (iconMarginGapWidth != -1) {
            if (iconMarginBitmap != null) {
                mBuilder.setSpan(new CustomIconMarginSpan(iconMarginBitmap, iconMarginGapWidth,
                        alignIconMargin), start, end, flag);
            } else if (iconMarginDrawable != null) {
                mBuilder.setSpan(new CustomIconMarginSpan(iconMarginDrawable, iconMarginGapWidth,
                        alignIconMargin), start, end, flag);
            } else if (iconMarginUri != null) {
                mBuilder.setSpan(new CustomIconMarginSpan(iconMarginUri, iconMarginGapWidth,
                        alignIconMargin), start, end, flag);
            } else if (iconMarginResourceId != -1) {
                if (iconMaxWidth != -1 && iconMaxHeight != -1){
                    mBuilder.setSpan(new CustomIconMarginSpan(iconMarginResourceId,
                            iconMarginGapWidth, alignIconMargin, iconMaxWidth, iconMaxHeight), start, end, flag);
                }else {
                    mBuilder.setSpan(new CustomIconMarginSpan(iconMarginResourceId,
                            iconMarginGapWidth, alignIconMargin), start, end, flag);
                }
            }
        }
        if (fontSize != -1) {
        ...
        if (blurRadius != -1) {
            mBuilder.setSpan(new MaskFilterSpan(new BlurMaskFilter(blurRadius, style)), start,
                    end, flag);
        }
        if (shader != null) {
    ...
    class CustomLineHeightSpan extends CharacterStyle
        ...
        @Override
        public void chooseHeight(final CharSequence text, final int start, final int end, final
        int spanstartv, final int v, final Paint.FontMetricsInt fm) {
    ...
    class CustomIconMarginSpan implements LeadingMarginSpan, android.text.style.LineHeightSpan {
        ...
        private CustomIconMarginSpan(final Drawable drawable, final int pad, final int
                verticalAlignment) {
        ...
        private CustomIconMarginSpan(final int resourceId, final int pad, final int
                verticalAlignment) {
        ...
        private CustomIconMarginSpan(final int resourceId, final int pad, final int
                verticalAlignment, int maxWidth, int maxHeight) {
            mBitmap = ImageUtil.getBitmap(resourceId, maxWidth, maxHeight);
            mPad = pad;
            mVerticalAlignment = verticalAlignment;
        }
    ...
    class CustomImageSpan extends CustomDynamicDrawableSpan {
        ...
        public Drawable getDrawable() {
            ...
            if (mDrawable != null) {
                ...
                Bitmap bitmap;
                try {
                    InputStream is = Utils.getApp().getContentResolver().openInputStream
                            (mContentUri);
    ...
    abstract class CustomDynamicDrawableSpan extends ReplacementSpan {
        ...
        public int getSize(@NonNull final Paint paint, final CharSequence text,
            ...
            final int fontHeight = (int) (paint.getFontMetrics().descent - paint.getFontMetrics()
                    .ascent);
    ...
    class ShadowSpan extends CharacterStyle implements UpdateAppearance {
        ...
        private ShadowSpan(final float radius, final float dx, final float dy, final int
                shadowColor) {
            this.radius = radius;
            this.dx = dx;
            this.dy = dy;
